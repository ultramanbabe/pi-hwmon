...
A FastAPI/WebSocket server runs on the PC and reads CPU, GPU, RAM, drives, and network stats via sysfs / RAPL / `smartctl`. A Raspberry Pi Zero 2W with a 7″ 1024×600 HDMI panel runs Chromium in kiosk mode and shows the dashboard at 60 fps, with sparklines that scroll on the compositor instead of repainting.
